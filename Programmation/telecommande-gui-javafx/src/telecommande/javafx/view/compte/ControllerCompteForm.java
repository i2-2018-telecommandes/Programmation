...
public class ControllerCompteForm {
	...
	public void init() {
		...
		compteVue.getRoles().addListener((ListChangeListener<String>) c -> {
			c.next();
		...
		// Affichage
		listViewRoles.setCellFactory(CheckBoxListCell.forListView((ItemRole item) -> item.choisiProperty()));
	...
	private void actualiserListeItemRoles() {
		Collections.sort(itemRoles, (Comparator<ItemRole>) (i1, i2) -> {
	...
	private class ItemRole {
		...
		public ItemRole(String role, boolean present) {
			...
			choisi.addListener((ChangeListener<Boolean>) (obs, oldValue, newValue) -> {
				if (newValue) {
