...
public class ControllerFournisseurListe {
	...
	public void init() {
		 ...
		 * ListCell<Fournisseur,String>() {
		 * 
		 * @Override protected void updateItem(Fournisseur item, boolean empty) {
		 * super.updateItem(item, empty); if (item == null) { setText(null); } else {
		 * setText(item.nomProperty().get() ); } } }; });
		 * 
		 * columnreference.setCellFactory( (list) -> { return new ListCell<Fournisseur,
		 * String>() {
		 * 
		 * @Override protected void updateItem(Fournisseur item, boolean empty) {
		 * super.updateItem(item, empty); if (item == null) { setText(null); } else {
		...
		tableView.getSelectionModel().getSelectedItems().addListener((ListChangeListener<Fournisseur>) (c) -> {
			configurerBoutons();
		});
